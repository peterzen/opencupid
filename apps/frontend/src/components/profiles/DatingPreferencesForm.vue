...
const {
  age,
} = useProfileFields(props.modelValue)
...
const prefAge = computed({
  get: () => {
    ...
    return [
      hasCustomMin ? min : fallbackMin,
      hasCustomMax ? max : fallbackMax
    ]
  },
...
<template>
  <fieldset>
    ...
    <div class="mb-3">
      <div v-if="age">
        <label for="ageRange"
               class="form-text text-muted">Age range</label>
        <div class="d-flex flex-row align-items-center">
          <div class="fs-2 me-2 px-4">
            {{ prefAge[0] }} - {{ prefAge[1] }}
          </div>
          <div class="flex-grow-1 px-3">
            <vue-slider v-model="prefAge"
                        id="ageRange"
                        :dotSize="20"
                        :contained="true"
                        :tooltip="'none'"
    ...
    <div class="mb-3">
      <FormKit v-model="formData.prefGender!"
               type="checkbox"
               label=""
               :options="genders"
               help=""
    ...
    <div class="mb-3">
      <FormKit v-model="formData.prefKids!"
               type="checkbox"
               label=""
